...
static int wireless_fw_update(struct battery_chg_dev *bcdev, bool force)
{
	...
	if (fw->size < bcdev->wls_fw_size_min) {
		...
		maj_ver = le16_to_cpu(*(__le16 *)(fw->data + CPS_FW_MAJOR_VER_OFFSET));
		min_ver = le16_to_cpu(*(__le16 *)(fw->data + CPS_FW_MINOR_VER_OFFSET));
	} else if (strstr(bcdev->wls_fw_name, "9412")) {
